...
func SaveTrade(ctx context.Context, agentName string, trade *types.Trade) error {
	log.Debug().Str("order_id", trade.OrderID).Msg("Saving trade to database")
	...
	if trade.Quantity != nil {
		...
		log.Debug().Str("order_id", trade.OrderID).Msg("Trade quantity is nil, using 0")
		quantity = 0
	}
	...
	if trade.Price != nil {
		...
		log.Debug().Str("order_id", trade.OrderID).Msg("Trade price is nil, using 0")
		price = 0
	}
	...
	if trade.Amount != nil {
		...
		log.Debug().Str("order_id", trade.OrderID).Msg("Trade amount is nil, using 0")
		amount = 0
	}

	// insert the trade into the database
	query := `
		INSERT INTO trades (id, agent_name, symbol, quantity, amount, price, action, timestamp, order_id)
	...
	log.Debug().Str("order_id", trade.OrderID).Msg("Trade saved to database")
	return nil
}
